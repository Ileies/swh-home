...
<header class="fixed top-0 z-50 w-full border-b border-gray-100 bg-base-100">
		...
		<nav class="flex items-center justify-between">
			...
			<div class="hidden items-center space-x-8 md:flex">
				{#each Object.entries(cms.headerLinks) as [href, text]}
					<a {href} class="text-base-content transition-colors hover:text-primary">{text}</a>
				{/each}
				<a href={cms.demoUrl} target="_blank"><button class="btn btn-outline">Live Demo</button></a>
				<a href={cms.contactLink} target="_blank"
					><button class="btn btn-primary">{cms.cta}</button></a
				>
			</div>
